...
export const CodeEditor: React.FC<CodeEditorProps> = ({ initialCode, onUpdate, selectedBoard }) => {
    ...
    const handleTranslate = async () => {
        ...
        // START SIMULATION
        setTimeout(() => {
            // Mock AI Logic: Verify which tab is active and "translate" to others
            const newCode = { ...code };

            if (activeTab === 'cpp') {
                newCode.python = `# [Auto-Translated from C++]\nimport machine\nimport time\n\n# Setup\nled = machine.Pin(2, machine.Pin.OUT)\nprint("System Initialized")\n\n# Loop\nwhile True:\n    led.value(1)\n    time.sleep(1)\n    led.value(0)\n    time.sleep(1)`;
                newCode.javascript = `// [Auto-Translated from C++]\nlet led = 2;\nconsole.log("System Initialized");\n\nsetInterval(() => {\n  digitalWrite(led, !digitalRead(led));\n}, 1000);`;
            } else if (activeTab === 'python') {
                newCode.cpp = `// [Auto-Translated from Python]\nvoid setup() {\n  pinMode(2, OUTPUT);\n}\n\nvoid loop() {\n  digitalWrite(2, HIGH);\n  delay(1000);\n  digitalWrite(2, LOW);\n  delay(1000);\n}`;
                newCode.javascript = `// [Auto-Translated from Python]\nlet led = 2;\nsetInterval(() => {\n  toggle(led);\n}, 1000);`;
            }

            setCode(newCode);
            onUpdate(newCode[activeTab], activeTab, newCode);
            setIsTranslating(false);
            alert(`¡Código traducido exitosamente!\nRevisa las pestañas de ${activeTab === 'cpp' ? 'Python y JS' : 'C++ y JS'} para ver el resultado.`);
        }, 2000);
    };
    ...
    return (
        <div className="bg-slate-900 rounded-[1.5rem] overflow-hidden shadow-2xl border-4 border-slate-800 flex flex-col h-[650px] font-sans">
            ...
            <div className="flex items-center justify-between p-2 bg-slate-950 border-b border-slate-800">
                ...
                `}
                    >
                        <Wand2 size={13} className={isTranslating ? "animate-spin" : ""} />
                        {isTranslating ? 'Analizando...' : 'Traducir con IA'}
                        {!isTranslating && <span className="bg-white/20 px-1 rounded text-[8px] ml-1">BETA</span>}
                    </button>
                    <button
